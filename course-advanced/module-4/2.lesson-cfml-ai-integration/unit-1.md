...
**Expected output** — you will see something like this:

```html
<p><strong>Model says:</strong> ColdFusion is a rapid web development platform
built by Adobe that enables developers to build dynamic, data-driven web
applications using a tag-based scripting language called CFML.</p>
<p><em>Tokens: 38 | 4821 ms</em></p>
```

What each part means:
- **Model says:** — the one-sentence answer generated by phi3:mini
- **Tokens:** — how many tokens the model produced
- **ms** — how long the inference took (expect 2,000–15,000 ms on CPU)

If you see that output, ColdFusion successfully called Ollama over the private network and decoded the JSON response. The task box below will turn green automatically once the file exists and returns a non-empty response.
